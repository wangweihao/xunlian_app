...
encode(login, Record) ->
    iolist_to_binary([pack(1, required,
			   with_default(Record#login.mark, none), int32, []),
		      pack(2, required,
			   with_default(Record#login.selfaccount, none), string,
			   []),
		      pack(3, required, with_default(Record#login.id, none),
			   int32, [])]);
encode(sendmsg, Record) ->
    iolist_to_binary([pack(1, required,
			   with_default(Record#sendmsg.mark, none), int32, []),
		      pack(2, required,
		      ...
		      pack(5, required, with_default(Record#sendmsg.id, none),
			   int32, [])]);
encode(ack, Record) ->
    iolist_to_binary([pack(1, required,
			   with_default(Record#ack.mark, none), int32, []),
		      pack(2, required, with_default(Record#ack.msgid, none),
			   int32, [])]);
encode(quit, Record) ->
    iolist_to_binary([pack(1, required,
			   with_default(Record#quit.mark, none), int32, [])]);
encode(keepalive, Record) ->
    iolist_to_binary([pack(1, required,
			   with_default(Record#keepalive.mark, none), int32,
			   []),
		      pack(2, required,
...
decode(keepalive, Bytes) when is_binary(Bytes) ->
    Types = [{2, isalive, bool, []}, {1, mark, int32, []}],
    Decoded = decode(Bytes, Types, []),
